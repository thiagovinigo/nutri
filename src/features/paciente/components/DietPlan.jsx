...
import { Utensils, RefreshCw, X, ShoppingCart, Printer, Sparkles, Loader2 } from 'lucide-react';
...
export default function DietPlan({ activePatient }) {
  ...
  const handleGenerateRecipe = async (meal, mIdx) => {
    ...
    try {
      const foodsList = meal.foods ? meal.foods.map(f => `${f.amount}g de ${f.name}`).join(', ') : '';
      ...
      setRecipeModal({ title: 'Erro', content: 'Infelizmente não foi possível gerar a receita no momento. Verifique sua conexão e tente novamente.' });
    } finally {
...
const handleOpenSub = (food) => {
    ...
    if (!baseDbFood) { alert('Não foi possível encontrar opções de substituição automática para este item customizado. Por favor, consulte sua Nutricionista.'); return; }
  ...
  return (
    <div className="animate-pop-in" style={{ position: 'relative' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        ...
        {currentRecipe && (
          <button className="btn-3d" onClick={() => setShowShoppingList(true)} style={{ backgroundColor: '#10b981', color: 'var(--crm-surface)', border: 'none', padding: '8px 16px', borderRadius: '12px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', boxShadow: '0 4px 0 #059669' }}>
      ...
      {activePatient.recipes.length === 0 ? (
        <p style={{color: 'var(--crm-text-muted)'}}>Nenhum plano alimentar recebido ainda.</p>
      ) : (
      ...
      {subModal && createPortal(
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 999, display: 'flex', justifyContent: 'center', alignItems: 'flex-end' }} onClick={() => setSubModal(null)}>
          <div className="animate-pop-in" style={{ backgroundColor: 'var(--crm-surface)', width: '100%', maxWidth: '600px', borderRadius: '20px 20px 0 0', padding: '24px', maxHeight: '80vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <h3 style={{ margin: 0, color: 'var(--crm-text-main)' }}>Substituir {subModal.original.name}</h3>
              <button onClick={() => setSubModal(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '8px' }}><X size={20} color='var(--crm-text-muted)' /></button>
            </div>
            
            <p style={{ color: 'var(--crm-text-muted)', fontSize: '0.9rem', marginBottom: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              {subModal.alternatives.length === 0 && (
                <p style={{ textAlign: 'center', color: 'var(--crm-text-muted)' }}>Nenhuma substituição encontrada nesta categoria.</p>
              )}
            </div>
          </div>
        </div>,
      ...
      {showShoppingList && createPortal(
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 999, display: 'flex', justifyContent: 'center', alignItems: 'flex-end' }} onClick={() => setShowShoppingList(false)}>
          <div className="animate-pop-in" style={{ backgroundColor: 'var(--crm-surface)', width: '100%', maxWidth: '600px', borderRadius: '20px 20px 0 0', padding: '24px', maxHeight: '85vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <h3 style={{ margin: 0, color: 'var(--crm-text-main)', display: 'flex', alignItems: 'center', gap: '8px' }}><ShoppingCart color="#10b981" /> Lista de Supermercado</h3>
              <button onClick={() => setShowShoppingList(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '8px' }}><X size={20} color='var(--crm-text-muted)' /></button>
            </div>
            
            <div style={{ marginBottom: '20px', display: 'flex', gap: '12px', alignItems: 'center', backgroundColor: 'var(--crm-surface-2)', padding: '12px', borderRadius: '12px' }}>
              <label style={{ color: 'var(--crm-text-main)', fontWeight: 600 }}>Calcular compras para:</label>
            ...
            {Object.keys(shoppingListGroups).length === 0 ? (
              <p style={{ color: 'var(--crm-text-muted)' }}>Nenhum alimento estruturado encontrado na dieta para gerar a lista.</p>
            ) : (
                    ...
                    <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                      {shoppingListGroups[category].map((item, idx) => (
                        <li key={idx} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px dashed #e2e8f0' }}>
                          <span style={{ color: 'var(--crm-text-main)', fontWeight: 500 }}>{item.name}</span>
            ...
            <button className="btn-3d" onClick={() => window.print()} style={{ marginTop: '24px', backgroundColor: '#3b82f6', color: 'var(--crm-surface)', border: 'none', padding: '12px', width: '100%', borderRadius: '12px', fontWeight: 'bold', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', cursor: 'pointer', boxShadow: '0 4px 0 #2563eb' }}>
      ...
      {(!activePatient.foodLogs || activePatient.foodLogs.length === 0) ? (
        <p style={{color: 'var(--crm-text-muted)'}}>Nenhuma refeição enviada para análise ainda.</p>
      ) : (
        <div style={{display: 'flex', flexDirection: 'column', gap: '16px'}}>
          {activePatient.foodLogs.slice().reverse().map((log, idx) => (
            <div key={idx} style={{...styles.card, flexDirection: 'column', alignItems: 'stretch', borderColor: log.type === 'extra' ? '#f59e0b' : '#3b82f6'}}>
              <div style={{display: 'flex', justifyContent: 'space-between', marginBottom: '8px'}}>
                <strong style={{color: 'var(--crm-text-main)'}}>{log.mealName}</strong>
                <span style={{fontSize: '0.85rem', color: 'var(--crm-text-muted)'}}>{log.date} às {log.time}</span>
              </div>
              <p style={{margin: '0', fontSize: '0.95rem', color: 'var(--crm-text-main)', whiteSpace: 'pre-wrap'}}>{log.log}</p>
            </div>
          ))}
        </div>
      )}
...
const styles = {
  sectionTitle: { fontSize: '1.2rem', fontWeight: '800', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--crm-text-main)' },
  card: { backgroundColor: 'var(--crm-surface)', borderRadius: '20px', padding: '20px', border: '2px solid #e2e8f0', boxShadow: '0 4px 0 #cbd5e1', marginBottom: '16px', display: 'flex' }
};
