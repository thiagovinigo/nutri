...
- [x] **PRD & Arquitetura de Integração:** Decisão tomada em 10/08/2026 — segue com Evolution API/Baileys (self-hosted no Render) enquanto o produto valida com poucos pacientes; risco de banimento do número é real (não-oficial) mas aceitável nesse estágio.
- [ ] **Gatilho de migração pra Meta Cloud API (oficial):** Migrar **antes** de passar de ~50-100 pacientes ativos reais recebendo lembretes proativos, ou antes de cobrar do nutricionista pelo módulo de WhatsApp como diferencial pago — o que vier primeiro. Motivo: Cloud API elimina o risco de ban arbitrário (canal sancionado, limite de conversas cresce automaticamente com a qualidade da conta), mas exige verificação de empresa no Meta Business Manager, portar o número, templates de mensagem pré-aprovados (obrigatório pra mensagem iniciada pela empresa, tipo lembrete) e reescrever `api/whatsapp-ai.js`/`whatsapp-webhook.js`/`cron-reminders.js` pro formato de payload do Cloud API (diferente do Evolution API). Evento que motivou o registro: número do bot ficou restrito 5h pelo WhatsApp em 10/08 por volume de testes automatizados no mesmo dia (não foi uso real, mas expôs o risco estrutural).
