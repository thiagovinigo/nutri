...
// Add selectedPlanDay state if missing
...
// Replace the entire r.meals?.map block
const rxBlockToReplace = /\{r\.meals\?\.map\(\(m, mIdx\) => \([\s\S]*?<\/div>\s*\)\)\}\s*\{r\.supplements && \(/;

const newBlock = `{maxDays > 0 && (
...
if (rxBlockToReplace.test(code)) {
  code = code.replace(rxBlockToReplace, newBlock);
} else {
  console.log("Failed to match rxBlockToReplace");
}

// Add maxDays calculation
const rxMapStart = /\{activePatient\.recipes\.slice\(\)\.reverse\(\)\.map\(\(r, idx\) => \(/;
const mapStartInsert = `{activePatient.recipes.slice().reverse().map((r, idx) => {
...
if (rxMapStart.test(code)) {
  code = code.replace(rxMapStart, mapStartInsert);
}

// Fix map end parenthesis
const rxOuterEnd = /<\/div>\s*\)\)\}/;
if (rxOuterEnd.test(code)) {
  code = code.replace(rxOuterEnd, '</div>)})}')
}

fs.writeFileSync('src/features/paciente/components/DietPlan.jsx', code, 'utf8');
console.log("DietPlan.jsx navigation+flipcard patched successfully");
