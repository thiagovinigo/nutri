...
if (code.includes(oldHandleGen)) {
  ...
  console.log("Could not find the target inside handleGenerateRecipe. It may already be patched.");
}
...
// 3. Fix applySub to use Deep Clone
const oldApplySub = `  const applySub = (alt, replaceAll) => {
    if (!activePatient) return;
    const newRecipes = [...(activePatient.recipes || [])];`;

const newApplySub = `  const applySub = (alt, replaceAll) => {
    if (!activePatient) return;
    // Deep clone to ensure React re-renders and Firebase gets a clean object without reference mutation issues
    const newRecipes = JSON.parse(JSON.stringify(activePatient.recipes || []));`;

if (code.includes(oldApplySub)) {
  code = code.replace(oldApplySub, newApplySub);
}

// 4. Fix UX of "Somente Hoje" button
const oldButton = `<button className="btn-3d" onClick={() => applySub(alt, false)} style={{ fontSize: '0.8rem', padding: '6px 12px', backgroundColor: 'var(--crm-surface-2, #f1f5f9)', color: 'var(--crm-text-main)', border: '1px solid var(--glass-border)', cursor: 'pointer' }}>Somente Hoje</button>`;
const newButton = `<button className="btn-3d" onClick={() => applySub(alt, false)} style={{ fontSize: '0.8rem', padding: '6px 12px', backgroundColor: 'transparent', color: '#3b82f6', border: '1px solid #3b82f6', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>Somente Hoje</button>`;
...
console.log('All fixes applied to DietPlan.jsx');
